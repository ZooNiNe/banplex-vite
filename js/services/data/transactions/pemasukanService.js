...
export async function handleAddPemasukan(formData, type) {
    ...
    try {
        ...
        const date = new Date(formData['pemasukan-tanggal'] || formData['date']);
        ...
        if (type === 'termin') {
            ...
            data = { id: generateUUID(), amount, date, projectId, description: `Termin ${projectName}`, notes, isDeleted: 0, syncState: 'pending_create', ...createdByInfo };
            ...
            data = {
                id: generateUUID(), creditorId, totalAmount: amount, description: `Pinjaman dari ${creditorName}`,
                date, status: 'unpaid', paidAmount: 0, isDeleted: 0, syncState: 'pending_create',
                interestType, rate, tenor, notes, ...createdByInfo
            };
         ...
         switch(type) {
            case 'termin':
                ...
                dataToUpdate = {
                    ...
                    projectId: form.elements['pemasukan-proyek']?.value,
                    notes
                };
                break;
