...
function createIcon(iconName, size = 22, classes = '') {
    ...
    const icons = {
        ...
        checklist: `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check-check ${classes}"><path d="M18 6 7 17l-5-5"/><path d="m22 10-7.5 7.5L13 16"/></svg>`, // Nama ikon diubah ke check-check
        ...
        // Special payments icon used on Reports page to open Simulation page
        ...
        'check-check': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check-check ${classes}"><path d="M18 6 7 17l-5-5"/><path d="m22 10-7.5 7.5L13 16"/></svg>`, // Icon untuk check-check
    };
...
export function createPageToolbarHTML({ title, searchId, searchPlaceholder, actions = [], isTransparent = false }) {
    ...
    const pagesWithoutMoreVert = new Set(['pengeluaran', 'hrd_applicants_form', 'hrd_applicants', 'file_storage', 'file_storage_form', 'pengaturan', 'jurnal', 'dashboard', 'laporan', 'absensi', 'simulasi', 'log_aktivitas', 'stok', 'master_data', 'pemasukan_form']);
